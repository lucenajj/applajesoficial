...
export const CapaProductsPage = () => {
  const [products, setProducts] = useState<Capa[]>([]);
  const [open, setOpen] = useState(false);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [editMode, setEditMode] = useState(false);
  const [selectedProduct, setSelectedProduct] = useState<Capa | null>(null);
  const { control, handleSubmit, reset, setValue, watch } = useForm<CapaFormData>({
    defaultValues: {
      nome: '',
      tipo: '',
      dimensoes: '',
  ...
  const fetchProducts = async () => {
    ...
    if (error) {
      console.error('Error fetching Capa products:', error);
      setError('Falha ao carregar produtos Capa. Por favor, tente novamente mais tarde.');
      return;
    }
  ...
  const handleEditProduct = (product: Capa) => {
  ...
  const handleDeleteProduct = (product: Capa, event: React.MouseEvent) => {
  ...
  const confirmDelete = async () => {
    ...
    if (error) {
      console.error('Error deleting Capa product:', error);
      setError('Falha ao excluir produto Capa. Por favor, tente novamente mais tarde.');
      return;
    }
  ...
  const onSubmit = async (data: CapaFormData) => {
    ...
    if (editMode && selectedProduct) {
      ...
      if (error) {
        console.error('Error updating Capa product:', error);
        setError('Falha ao atualizar produto Capa. Por favor, tente novamente mais tarde.');
        return;
      }
    } else {
      const { error } = await supabase.from('capa_products').insert([formattedData]);

      if (error) {
        console.error('Error creating Capa product:', error);
        setError(`Falha ao criar produto Capa: ${error.message}`);
        return;
      }
    }
    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      {loading ? (
        ...
        <TableContainer component={Paper} sx={{ mb: 4, maxHeight: 'calc(100vh - 250px)', overflow: 'auto' }}>
          <Table stickyHeader aria-label="produtos capa table">
            <TableHead>
              <TableRow>
                ...
                <TableRow>
                  <TableCell colSpan={7} align="center">
                    <Typography variant="body1" color="textSecondary" py={2}>
                      Nenhum produto Capa cadastrado
                    </Typography>
                  </TableCell>
                </TableRow>
              ) : (
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="md" fullWidth>
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogTitle>{editMode ? 'Editar Produto Capa' : 'Novo Produto Capa'}</DialogTitle>
